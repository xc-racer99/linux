...
void modem_update_state(struct modemctl *mc)
{
	...
	if (mc->fmt_rx.avail)
		pm_stay_awake(mc->cmd_pipe.dev.this_device);
	else
		pm_relax(mc->cmd_pipe.dev.this_device);

	mc->rfs_tx.avail = fifo_space(&mc->rfs_tx);
	mc->rfs_rx.avail = fifo_count(&mc->rfs_rx);
	if (mc->rfs_rx.avail)
		pm_stay_awake(mc->rfs_pipe.dev.this_device);
	else
		pm_relax(mc->rfs_pipe.dev.this_device);
...
void modem_update_pipe(struct m_pipe *pipe)
{
	...
	if (pipe->rx->avail)
		pm_stay_awake(pipe->dev.this_device);
	else
		pm_relax(pipe->dev.this_device);
...
static int modem_pipe_read(struct m_pipe *pipe, struct modem_io *io)
{
	...
	ret = pipe->pull_header(io, hdr, pipe->mc->data->quirks & QUIRK_COUNTING_CONTROL);
	if (ret)
		return ret;
...
static void handle_raw_rx(struct modemctl *mc)
{
	...
	while (fifo_read(&mc->raw_rx, &raw, sizeof(raw)) == sizeof(raw)) {
		...
		if (unlikely(!(raw.channel >= RAW_CH_VNET0 && raw.channel <
				NETDEV_TO_CHANNEL_ID(mc->data->num_pdp_contexts)))) {
...
void modem_handle_io(struct modemctl *mc)
{
	...
	for (i = 0; i < mc->data->num_pdp_contexts; i++) {
...
static int pull_fmt_header(struct modem_io *io, void *header, bool counting)
{
	struct fmt_hdr *fh = header;
...
static int pull_rfs_header(struct modem_io *io, void *header, bool counting)
{
	struct rfs_hdr *rh = header;
...
static int modem_pipe_register(struct m_pipe *pipe, const char *devname)
{
	...
	ret = misc_register(&pipe->dev);
	if (ret < 0) {
		pr_err("%s: fail registering misc dev %s", __func__, devname);
	}

	return device_init_wakeup(pipe->dev.this_device, true);
}
...
int modem_io_init(struct modemctl *mc, void __iomem *mmio)
{
	...
	mc->ndev = kmalloc(sizeof(struct net_device *) * mc->data->num_pdp_contexts,
			 GFP_KERNEL);
	if (!mc->ndev) {
		pr_err("memory allocation failed for netdev\n");
		return -ENOMEM;
	}
	for (i = 0, ch_id = RAW_CH_VNET0; i < mc->data->num_pdp_contexts;
			i++, ch_id++) {
		...
		if (mc->ndev[i]) {
			...
			device_init_wakeup(&mc->ndev[i]->dev, true);
		} else {
