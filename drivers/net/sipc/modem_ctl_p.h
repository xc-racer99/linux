...
struct modemctl {
	...
	struct sipc_link fmt_ops;
	struct sipc_link rfs_ops;
	struct sipc_link raw_ops;
	...
	struct gpio_desc *gpio_int_resout;
	struct gpio_desc *gpio_cp_pwr_rst;

	enum modemctl_variant variant;
...
#define fmt_ops_to_mc(ops) container_of(ops, struct modemctl, fmt_ops)
#define raw_ops_to_mc(ops) container_of(ops, struct modemctl, raw_ops)
#define rfs_ops_to_mc(ops) container_of(ops, struct modemctl, rfs_ops)
