...
static int modem_off(struct modemctl *mc)
{
	...
	if (mc->data->variant == STE_M5730) {
		...
		gpiod_direction_output(mc->gpio_cp_reset, 0);

		if (!gpiod_get_value(mc->gpio_resout) && !gpiod_get_value(mc->gpio_cp_pwr_rst)) {
			if (regulator_is_enabled(mc->cp_32khz_regulator)) {
...
static irqreturn_t resout_irq_handler(int irq, void *_mc)
{
	...
	if (!gpiod_get_value(mc->gpio_resout) && gpiod_get_value(mc->gpio_phone_on))
...
static irqreturn_t cp_pwr_rst_irq_handler(int irq, void *_mc)
{
	...
	if (!gpiod_get_value(mc->gpio_cp_pwr_rst) && gpiod_get_value(mc->gpio_phone_on))
		pm_wakeup_event(mc->dev.this_device, 600 * HZ);
...
static void modemctl_handle_offline(struct modemctl *mc, unsigned cmd)
{
	...
	case MODEM_BOOTING_NORMAL:
		if (cmd == MODEM_MSG_BINARY_DONE) {
			 ...
			 */
			if (mc->data->quirks & QUIRK_INIT_END_EARLY)
...
static irqreturn_t modemctl_mbox_irq_handler(int irq, void *_mc)
{
	...
	} else if (mc->data->quirks & QUIRK_RELEASE_SEM && mmio_sem(mc) == 0) {
...
static const struct modemctl_data xmm6160_data  = {
	.variant = INTEL_XMM6160,
	.quirks = QUIRK_RELEASE_SEM | QUIRK_COUNTING_CONTROL |
			QUIRK_INIT_END_EARLY,
	.num_pdp_contexts = 3,
};

static const struct modemctl_data ste_m5730_data  = {
	.variant = INTEL_XMM6160,
	.quirks = 0,
	.num_pdp_contexts = 3,
};
...
static int modemctl_probe(struct platform_device *pdev)
{
	...
	if (!pdev->dev.of_node) {
		dev_err(&pdev->dev, "must be instantiated via DT");
		return -EINVAL;
	}
	np = pdev->dev.of_node;

	mc->data = (struct modemctl_data *) of_match_node(modemctl_of_match, np);

	mc->gpio_phone_active = devm_gpiod_get(dev, "phone_active", 0);
	if (IS_ERR(mc->gpio_phone_active)) {
		pr_err("no phone_active gpio");
		return PTR_ERR(mc->gpio_phone_active);
	}

	mc->gpio_pda_active = devm_gpiod_get(dev, "pda_active", 0);
	if (IS_ERR(mc->gpio_pda_active)) {
		pr_err("no pda_active gpio");
		return PTR_ERR(mc->gpio_pda_active);
	}

	mc->gpio_cp_reset = devm_gpiod_get(dev, "cp_reset", 0);
	...
	if (mc->data->variant == STE_M5730) {
		mc->gpio_phone_on = devm_gpiod_get(dev, "phone_on", 0);
		if (IS_ERR(mc->gpio_phone_on)) {
			pr_err("no phone_on gpio");
			return PTR_ERR(mc->gpio_phone_on);
		}

		mc->gpio_resout = devm_gpiod_get(dev, "resout", 0);
		if (IS_ERR(mc->gpio_resout)) {
			pr_err("no resout gpio");
			return IS_ERR(mc->gpio_resout);
		}

		mc->irq_resout = gpiod_to_irq(mc->gpio_resout);
		if (mc->irq_resout < 0) {
			pr_err("no resout irq");
			return mc->irq_resout;
		}

		r = devm_request_irq(&pdev->dev, mc->irq_resout, resout_irq_handler,
		...
		mc->gpio_cp_pwr_rst = devm_gpiod_get(dev, "cp_pwr_rst", 0);
		...
		r = devm_request_irq(&pdev->dev, mc->irq_cp_pwr_rst, cp_pwr_rst_irq_handler,
	...
	mc->irq_bp = gpiod_to_irq(mc->gpio_phone_active);
	mc->irq_mbox = platform_get_irq(pdev, 0);
